...
#define xprintf_COLOR(c, ...) _Pragma ("GCC warning \"'xprintf_COLOR' macro is deprecated\"")   do {                            \
									switch (c) {                \
										case E_FONT_BLACK:      \
										TERMINAL_FONT_BLACK();  \
										break;                  \
...
#define TERMINAL_FONT_BLACK()       xprintf("\033[1;30m")
#define TERMINAL_FONT_L_RED()       xprintf("\033[0;31m")    /* light red */
#define TERMINAL_FONT_RED()         xprintf("\033[1;31m")    /* red */
#define TERMINAL_FONT_GREEN()       xprintf("\033[1;32m")
#define TERMINAL_FONT_YELLOW()      xprintf("\033[1;33m")
#define TERMINAL_FONT_BLUE()        xprintf("\033[1;34m")
#define TERMINAL_FONT_PURPLE()      xprintf("\033[1;35m")
#define TERMINAL_FONT_CYAN()        xprintf("\033[1;36m")
#define TERMINAL_FONT_WHITE()       xprintf("\033[1;37m")
...
#define TERMINAL_BACK_BLACK()       xprintf("\033[1;40m")
#define TERMINAL_BACK_L_RED()       xprintf("\033[0;41m")    /* light red */
#define TERMINAL_BACK_RED()         xprintf("\033[1;41m")    /* red */
#define TERMINAL_BACK_GREEN()       xprintf("\033[1;42m")
#define TERMINAL_BACK_YELLOW()      xprintf("\033[1;43m")
#define TERMINAL_BACK_BLUE()        xprintf("\033[1;44m")
#define TERMINAL_BACK_PURPLE()      xprintf("\033[1;45m")
#define TERMINAL_BACK_CYAN()        xprintf("\033[1;46m")
#define TERMINAL_BACK_WHITE()       xprintf("\033[1;47m")
#define TERMINAL_BACK_DEFAULT()		TERMINAL_BACK_BLACK()

/* terminal clear end */
#define TERMINAL_CLEAR_END()        xprintf("\033[K")

/* terminal clear all */
#define TERMINAL_DISPLAY_CLEAR()    xprintf("\033[2J")

/* cursor move up */
#define TERMINAL_MOVE_UP(x)         do{ if(x>0) xprintf("\033[%dA", (x)); }while(0)

/* cursor move down */
#define TERMINAL_MOVE_DOWN(x)       do{ if(x>0) xprintf("\033[%dB", (x)); }while(0)

/* cursor move left */
#define TERMINAL_MOVE_LEFT(y)       do{ if(y>0) xprintf("\033[%dD", (y)); }while(0)

/* cursor move right */
#define TERMINAL_MOVE_RIGHT(y)      do{ if(y>0) xprintf("\033[%dC", (y)); }while(0)

/* cursor move to */
#define TERMINAL_MOVE_TO(x, y)      xprintf("\033[%d;%dH", (x), (y))

/* cursor reset */
#define TERMINAL_RESET_CURSOR()     xprintf("\033[H")

/* cursor invisible */
#define TERMINAL_HIDE_CURSOR()      xprintf("\033[?25l")

/* cursor visible */
#define TERMINAL_SHOW_CURSOR()      xprintf("\033[?25h")

/* reverse display */
#define TERMINAL_HIGHLIGHT()       xprintf("\033[7m")
#define TERMINAL_UN_HIGHLIGHT()    xprintf("\033[27m")
